...
class TiendaNubeScraper {
  ...
  async scrapeStore(store) {
    ...
    try {
      ...
      // Solo bloquear imágenes pesadas, permitir JavaScript y CSS crítico
      await page.setRequestInterception(true);
      page.on('request', (req) => {
        const resourceType = req.resourceType();
        const url = req.url();
        
        // Bloquear solo imágenes muy pesadas y fuentes no críticas
        if (resourceType === 'image' && (url.includes('.jpg') || url.includes('.png') || url.includes('.gif') || url.includes('.webp'))) {
          req.abort();
        } else if (resourceType === 'font' && !url.includes('critical')) {
          req.abort();
        } else {
          // Permitir JavaScript, CSS, XHR, fetch - críticos para TiendaNube
          req.continue();
        }
      });
      ...
      if (this.database && results.products.length > 0) {
        console.log(`💾 Guardando ${results.products.length} productos en base de datos...`);
        
        for (const product of results.products) {
          try {
            const result = await this.database.upsertProduct(product);
            if (result.isNew) {
              newProducts++;
            ...
            console.error(`❌ Error guardando producto ${product.name}:`, error.message);
          }
        }
        
        // Actualizar información de la tienda
        await this.database.updateStoreLastScraped(store.id, savedCount);
        
        console.log(`💾 Base de datos actualizada: ${newProducts} nuevos, ${updatedProducts} actualizados, ${savedCount} total`);
      }
    ...
    while (hasNextPage && currentPage <= maxPages) {
      try {
        ...
        const response = await page.goto(url, { 
          waitUntil: 'networkidle2', // Esperar a que la red esté tranquila (JavaScript cargado)
  ...
  async waitForProducts(page) {
    try {
      // Esperar a que aparezcan elementos de productos
      ...
      // Esperar específicamente a que se carguen precios (crítico para TiendaNube)
      await page.waitForFunction(() => {
        const priceSelectors = ['.js-price-display', '.price-display', '.price', '.money'];
        return priceSelectors.some(selector => {
          const priceElements = document.querySelectorAll(selector);
          return Array.from(priceElements).some(el => el.textContent && el.textContent.trim().length > 0);
        });
      }, { timeout: 10000 });
      
      console.log('✅ Productos y precios cargados');
      
    } catch (error) {
      console.log('⏱️ Timeout esperando productos/precios, continuando...');
    }
  ...
  async extractProductsFromPage(page, storeData) {
    ...
    $(productSelector).each((index, element) => {
      try {
        ...
        if (product && product.name && product.current_price > 0) {
          products.push(product);
          console.log(`✅ Producto ${index + 1}: "${product.name}" - $${product.current_price}`);
        } else {
          console.log(`⚠️ Producto ${index + 1}: datos incompletos - name: "${product?.name}", price: ${product?.current_price}`);
        }
      } catch (error) {
  ...
  // Método para scraping múltiples tiendas específicas (usado por demo)
  async scrapeMultipleStores(targetStores) {
    const results = [];
    
    console.log(`🏪 Iniciando scraping de ${targetStores.length} tiendas seleccionadas`);
    ...
    for (const store of targetStores) {
      ...
      if (targetStores.indexOf(store) < targetStores.length - 1) {
    ...
    const totalSaved = results.reduce((sum, r) => sum + (r.products_saved || 0), 0);
    
    console.log(`📊 Resumen: ${successful}/${targetStores.length} tiendas exitosas, ${totalProducts} productos extraídos, ${totalSaved} guardados`);
  ...
  async scrapeAllStores() {
    // Get stores from stores configuration
    const { stores } = require('./stores');
    const activeStores = stores.filter(store => store.active);
    
    const results = [];
    
    console.log(`🏪 Iniciando scraping de ${activeStores.length} tiendas`);
    ...
    for (const store of activeStores) {
      ...
      // Pausa entre tiendas para ser respetuosos
      if (activeStores.indexOf(store) < activeStores.length - 1) {
        console.log(`⏸️ Pausa de ${this.requestDelay}ms antes de la siguiente tienda`);
        await this.delay(this.requestDelay);
      }
    }
    ...
    const totalProducts = results.reduce((sum, r) => sum + (r.products?.length || 0), 0);
    const totalSaved = results.reduce((sum, r) => sum + (r.products_saved || 0), 0);
    const totalNew = results.reduce((sum, r) => sum + (r.new_products || 0), 0);
    const totalUpdated = results.reduce((sum, r) => sum + (r.updated_products || 0), 0);
    
    console.log(`📊 Resumen: ${successful}/${activeStores.length} tiendas exitosas, ${totalProducts} productos extraídos, ${totalSaved} guardados en DB`);
